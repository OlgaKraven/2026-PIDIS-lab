import {resolveLab} from './resolveLab'
...
const byteCache=new Map<string,Promise<Uint8Array>>();const fetchBytes=(path:string)=>{let p=byteCache.get(path);if(!p){p=fetch(import.meta.env.BASE_URL+path).then(async r=>{if(!r.ok)throw Error(`Не найден файл ${path}`);return new Uint8Array(await r.arrayBuffer())}).catch(e=>{byteCache.delete(path);throw e});byteCache.set(path,p)}return p}
...
async function buildFiles({labs,area,profile,renderPage}:{labs:Lab[];area:SubjectArea;profile:QualityProfile;renderPage:(lab:Lab)=>string}){
 ...
 for(const baseLab of labs){
 const lab=resolveLab(baseLab,area);
 ...
 const prerequisite=doc.createElement('p');prerequisite.textContent=labMethodology[lab.id].sequence.previous;body.append(prerequisite);
 for(const id of ['situation','goal','profile','inputs','theory','example','task','self-check','sequence','lms-submit']){
