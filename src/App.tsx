...
function useRoute() {
  const readHash = () => {const h=window.location.hash;if(h)return h;const id=new URLSearchParams(window.location.search).get('lab');return id?'#/lab/'+id:'#/'}
...
function CourseApp() {
  ...
  const [subjectAreaId, setSubjectAreaId] = useState(() => {
    const saved = Number(window.localStorage.getItem('pidis-lab-v2-subject-area'))
  ...
  const selectSubjectArea = (id: number) => {
    setSubjectAreaId(id)
    window.localStorage.setItem('pidis-lab-v2-subject-area', String(id))
  }
        ...
        <section className="catalog-section" id="labs" aria-labelledby="labs-title">
          ...
          {visibleLabs.length ? (
            <div className="lab-grid">
              {visibleLabs.map((lab) => <LabCard key={lab.id} lab={lab} area={subjectArea} profile={profile} />)}
            </div>
          ) : (
...
function LabCard({ lab,area,profile }: { lab: Lab;area:SubjectArea;profile:QualityProfile }) {
  return (
    <article className="lab-card">
      ...
      <p className="topic-line">{lab.topicTitle}</p>
      <h3>{lab.title}</h3>
...
function LabPage({ lab: baseLab, subjectArea, profile, onSubjectAreaChange }: { lab: Lab; subjectArea: SubjectArea; profile: QualityProfile; onSubjectAreaChange: (id: number) => void }) {
  const lab=resolveLab(baseLab,subjectArea)
  const [packageStatus, setPackageStatus] = useState<'idle' | 'preparing' | 'error'>('idle')
  const previous = labs.find((item) => item.order === lab.order - 1)
  const next = labs.find((item) => item.order === lab.order + 1)
  ...
  const methodology = labMethodology[lab.id]
        ...
        <div className="lab-page-grid">
          <article className="lab-content">
            ...
            <ContentSection id="lms-submit" number="03" label="Отчёт и LMS" title="Требования к отчёту и сдаче" icon={<GraduationCap aria-hidden="true" />}>
              <p><strong>Один заполненный редактируемый DOCX-файл.</strong></p><h3>Критерии оценивания · максимум {lab.points} баллов</h3><ResponsiveTable data={{columns:["Результат","Что оценивается","Баллы"],rows:lab.rubric}}/><p>Частичное выполнение оценивается по независимым результатам в пределах указанных баллов. Объяснение оснований и проверки решения входит в оценку.</p><h3>Требования к Word-файлу</h3><Checklist items={lab.wordRequirements.map(labText)} />
  ...
  return (
    <ol className="task-protocol">
      {actions.map((action, index) => {
        ...
        return (
          <li key={action}>
            <h3>Шаг {index + 1}. {action}</h3><p className="step-theory">{guide.theory}</p>
            <dl>
